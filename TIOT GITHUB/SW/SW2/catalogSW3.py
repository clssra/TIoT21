...
class checkTime(threading.Thread):
    ...
    def run(self):
        ...
        for i in devices.keys():
            if (time.time() - devices.get(i).get('TimeStamp')) > 12:
            	devices.pop(i)
...
class catalog():
    ...
    def POST(self, *uri):
    	...
    	if uri[0] == 'devices':
            if len(json_body) == 3:
                timestamp = time.time()
                devices.update({json_body.get('ID'): {'EndPoints': json_body.get(
                    'EndPoints'), 'AvailResurces': json_body.get('AvailResurces'), 'TimeStamp': timestamp}})
            else:
                return "wrong number of params"
    	elif uri[0] == 'users':
            if len(json_body) == 4:
